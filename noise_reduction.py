...
from matplotlib import pyplot as plt     #for plot the image
import math                              #for math iopoerations
import glob                              #for acess the files from folder
import os                                #for file or folder handling
import shutil                            #for remove the folder with files



videopath=input("Enter any Path====")
print("Your Path is===",videopath)
#break video into frames with noisey frames with creating folder
...
vidcap1 = cv2.VideoCapture(videopath)
...
while success:
  ...
  vidcap1.set(cv2.CAP_PROP_POS_MSEC,(count)) #used to hold speed of frane generation
...
for x in range(count):      #take no. of frame frames from loop count
    
        plt.subplot(121),plt.imshow(f1[x]),plt.title('Original') #images with noise
        plt.xticks([]), plt.yticks([])
        plt.subplot(122),plt.imshow(f2[x]),plt.title('Blurred') #imges without noise
        plt.xticks([]), plt.yticks([])
        d=psnr(f1[x],f2[x])    #calling psnr calculation function
        print("psnr",d)      #print psnr of ever two frmES.i.e original and blured
